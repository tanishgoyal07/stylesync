...
# Apply the function to create a new column 'age-group' in user_data
user_data['age-group'] = user_data.apply(categorize_user_by_age_gender, axis=1)

# Preprocessing & Encoding
# Combine unique categories for consistent encoding space
combined_categories = pd.concat([
    user_data[['usage', 'category', 'articleType', 'age-group']],
    designer_data[['expert-category', 'expert-subCategory', 'experienced-in', 'age-group']].rename(
        columns={'expert-category': 'usage', 'expert-subCategory': 'category', 'experienced-in': 'articleType', 'age-group': 'age-group'}
    )
...
user_items_encoded = encoder.transform(user_data[['usage', 'category', 'articleType', 'age-group']]).toarray()
...
scaler = MinMaxScaler()
total_customers_normalized = scaler.fit_transform(designer_data[['total-customers-served']].values)
...
def calculate_similarity_with_age(user_idx, designer_idx, is_new_user=False, new_user_age_group=None):
    ...
    if is_new_user:
        age_similarity = 0.7 if new_user_age_group == designer_data.iloc[designer_idx]['age-group'] else 0
    else:
        age_similarity = 0.7 if user_data.iloc[user_idx]['age-group'] == designer_data.iloc[designer_idx]['age-group'] else 0
...
def recommend_designers_for_new_user(new_user_input):
    ...
    new_user_input['age-group'] = categorize_user_by_age_gender(new_user_input)
    
    new_user_encoded = encoder.transform(
        pd.DataFrame([new_user_input])[['usage', 'category', 'articleType', 'age-group']]
    ).toarray()
    ...
    for designer_idx, designer in designer_data.iterrows():
        if designer['availability'] == 'Yes':
            age_clothing_similarity = calculate_similarity_with_age(
                user_idx=None, designer_idx=designer_idx, is_new_user=True, new_user_age_group=new_user_input['age-group']
            )
